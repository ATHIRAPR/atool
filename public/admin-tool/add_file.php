...
if ($_SERVER ['REQUEST_METHOD'] === "GET") {
	...
	$download_file_nm = trim ( mb_substr ( mb_convert_encoding ( $_POST ['download_filename'], "UTF-8", $OUTPUT_ENCODING ), 0, 100 ) );
	$pdf_password = substr ( $_POST ['pdf_password'], 0, 50 );
	$title = mb_substr ( mb_convert_encoding ( $_POST ['title'], "UTF-8", $OUTPUT_ENCODING ), 0, 1000 );
	...
	if (! empty ( $download_file_nm )) {
		$download_file_nm = get_pdf_filename ( $download_file_nm ) . '.pdf';
	}
	$download_file_nm = str_replace ( '/', '／', $download_file_nm );
